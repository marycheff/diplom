...
class BadWordsService {
	...
	private async initBadWords(): Promise<void> {
		try {
			...
			for (const raw of words) {
				...
				// if (isRegex) {
				// 	// Замена псевдо-\w и экранирование вручную
				// 	try {
				// 		const pattern = word
				// 			.replace(/\\w\*/g, "[\\p{L}\\p{N}_]*")
				// 			.replace(/\\w\+/g, "[\\p{L}\\p{N}_]+")
				// 			.replace(/\\w/g, "[\\p{L}\\p{N}_]")
				// 			.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") // экранирование всего остального
				// 			.replace(/\\\[\\p\{L}\\\\p\{N}_]([*+]?)/g, "[\\p{L}\\p{N}_]$1") // возвращение \p блоков после экранирования
	...
	private normalizeText(text: string): string {
		...
		return text
			.replace(/[_\-*.\s]+/g, " ") // Замена разделителей на пробелы
