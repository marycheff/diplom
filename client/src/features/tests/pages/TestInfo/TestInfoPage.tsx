					...
					<div className={styles.blockHeader}>
						...
						<div className={styles.buttonContainer}>
							...
							<Button
								...
								tooltip={test.visibilityStatus === TestVisibilityStatus.HIDDEN ? "Опубликовать" : "Скрыть"}
							>
								...
								{isSettingsUpdating ? (
									...
									<>
										...
										<div className={styles.infoRow}>
											...
											<span className={styles.value}>{test.settings.shuffleQuestions ? "Да" : "Нет"}</span>
										</div>
										<div className={styles.infoRow}>
											<span className={styles.label}>Перемешивать варианты ответов</span>
											<span className={styles.value}>{test.settings.shuffleAnswers ? "Да" : "Нет"}</span>
										</div>
										<div className={styles.infoRow}>
											<span className={styles.label}>Разрешить повторное прохождение</span>
											<span className={styles.value}>{test.settings.allowRetake ? "Да" : "Нет"}</span>
										</div>
										<div className={styles.infoRow}>
											<span className={styles.label}>Лимит повторного прохождения</span>
											<span className={styles.value}>
												{test.settings.retakeLimit || <span className={styles.emptyField}>не указан</span>}
											</span>
										</div>
										<div className={styles.infoRow}>
											...
											<span className={styles.value}>
												{test.settings.inputFields && test.settings.inputFields.length > 0 ? (
													test.settings.inputFields.map((field) => PreTestUserDataLabels[field] || field).join(", ")
												) : (
				...
				<div className={styles.blockHeader}>
					...
					<div className={styles.buttonContainer}>
						<Button
							...
							className={styles.editBtn}
							tooltip={test.questions?.length && test.questions?.length > 0 ? "Редактировать" : "Добавить вопросы"}
						>
