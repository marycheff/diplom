...
const AllTestsPage = () => {
    ...
    const { getCacheKey, getCachedData, saveToCache, clearCache, cacheVersion, lastUpdateDate } =
        useCache<TestsListDTO>(useTestStore, "unmounted-tests")
    ...
    const fetchData = useCallback(
        async (currentPage: number, query?: string) => {
            ...
            if (query) {
                ...
                data = await getUnmoderatedTests(currentPage, limit)
            }
            if (data) {
                setTests(data.tests)
                setTotal(data.total)
        ...
        [getCacheKey, getCachedData, saveToCache, searchTests, getUnmoderatedTests, limit]
    )
